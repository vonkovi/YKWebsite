...
YvonKim.com is a personal portfolio site for Yvon Kim — electrical engineer, software engineer, and prompt engineer. It is a single scrolling page of **Projects** and nothing else. The work does the talking; there is no hero, no bio, no "about." The audience is the general public (and hiring managers).
...
src/                  ← Astro source
  content/
    projects/         ← MDX — one file per project (frontmatter + body renders on its /projects/<slug> page)
  content.config.ts   ← Zod schema: projects only (NO orgs, NO blog)
  components/
    Header.astro      ← Sticky nav: wordmark (left), social + résumé icons (right)
  ...
  pages/
    index.astro       ← Home — name-led project rows + client script (card nav + reveal)
...
Static Astro site built around a home page (`/`) plus a generated case-study page per project (`/projects/<slug>`), with one Astro Content Collection: `projects`. All content is MDX flat files — no database, no server runtime.

### Home page

`index.astro` renders the project rows directly (no tabs); a small inlined client script handles whole-card navigation and the staggered reveal.

- **Projects** — name-led rows from the `projects` collection, sorted by `order`. Each row: image well (left) + category label / name / 3-line-clamped description / `Source` link (right). The whole row links to that project's case-study page (`/projects/<slug>`) — the title is a link and a client-side handler navigates on any non-link click. Hairline dividers between rows.

### Detail pages

Project detail pages use `Header` in its **`subnav`** variant (wordmark + a `←` back-link; `variant="subnav"` + `backHref`/`backLabel` props) and the shared `.detail` / `.prose` styles.
...
- **Routes are `/` and `/projects/<slug>` only.** No `/projects` index page, no `/orgs`, no `/writing` or `/blog`. Projects link out to their own detail page; don't add other routes without being asked.
- **No Organizations, no Blog/Writing.** Both collections were removed (orgs in the redesign, blog afterward).
...
- **Images** live in `public/` (e.g. `public/projects/`) and are referenced by root-absolute path (`/projects/foo.png`). Use `image:` frontmatter for a project's row/hero; inside any MDX body use Markdown `![alt](/path)` or an HTML `<figure>`/`<figcaption>`. `.prose img` and `.prose figure` are styled in `global.css` (bordered, rounded, captioned). No `astro:assets` optimization — size images before adding.
- Light mode only — no dark mode toggle.
- Header icons: Email (mailto), GitHub, LinkedIn, X (x.com/vonkovi_ — where Yvon now posts writing, replacing the removed blog), then a divider, then Résumé (downloads `/Yvon-Kim-Resume.pdf`). Footer repeats the same social links plus Résumé as text links.
...
- **Typography:** IBM Plex Sans (display + body), IBM Plex Mono (small metadata/labels only — footer, category labels, project meta). No serif. Loaded via Google Fonts `<link>` in `BaseLayout`.
...
- **Accent = interaction, period.** `--navy` (IEEE navy) appears **only** on things you can act on — links, hover, résumé. Never decorative.
